...
//아이디어 입력 초기 설정
export const initinputidea = async (workspaceId: number): Promise<ApiResponse<IdeaData>> => {
    try {
...
//기술 스택 생성
export const inputtech = async (workspaceId: number, ideaInputId: number): Promise<ApiResponse<TechStack>> => {
    try {
        console.log("기술 스택 생성 api");
        const response = await api.post(`/workspaces/${workspaceId}/idea-input/${ideaInputId}/tech-stack`);
...
//메인 기능 생성
export const inputfunc = async (workspaceId: number, ideaInputId: number): Promise<ApiResponse<MainFunction>> => {
    try {
        console.log("메인 기능 생성 api");
        const response = await api.post(`/workspaces/${workspaceId}/idea-input/${ideaInputId}/main-function`);
        console.log("메인 기능 생성 :", response);
        return response.data;
    } catch (error: any) {
        console.error("🔴 [inputfunc] 기술스택 생성 실패:", error);
...
//아이디어 입력 조회
export const getidea = async (workspaceId: number): Promise<ApiResponse<IdeaData>> => {
    try {
...
//아이디어 입력 수정
export const putidea = async (workspaceId: number, ideadata: IdeaData): Promise<ApiResponse<IdeaData>> => {
    try {
        console.log("아이디어 조회 api");
        const response = await api.put(`/workspaces/${workspaceId}/idea-input/${ideadata.ideaInputId}`, {
