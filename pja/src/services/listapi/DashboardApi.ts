...
import type { myActionList, myProgress, processtime, taskimbalance } from "../../types/list";
...
export const getTaskImbalance = async (
    ...
    try {
        ...
        const response = await api.get(
            `/workspaces/${workspaceId}/task-imbalance`
        );
...
export const getTaskProcessTime = async (
    ...
    try {
        ...
        console.log("작업처리시간 그래프 :", response);
        return response.data;
    } catch (error: any) {
        console.error("🔴 [getTaskProcessTime] 작업처리시간 그래프 가져오기 실패:", error);
