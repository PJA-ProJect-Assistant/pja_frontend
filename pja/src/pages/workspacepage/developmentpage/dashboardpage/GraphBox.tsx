import {
    BarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, LineChart,
    Line, CartesianGrid
} from 'recharts';
import "./GraphBox.css"
...
import { getTaskImbalance, getTaskProcessTime } from '../../../../services/listapi/DashboardApi';
import type { Importance, processtime, Status, taskimbalance } from '../../../../types/list';
...
const TransformLineData = (data: processtime[]) => {
    const importances = Array.from(
        new Set(data.map((d) => d.importance))
    ).sort((a, b) => a - b);
    ...
    const LinechartData = importances.map((imp) => {
        ...
        userIds.forEach((uid) => {
            const item = data.find(
                (d) => d.userId === uid && d.importance === imp
            );
    ...
    useEffect(() => {
        const getimblance = async () => {
            try {
                ...
                // 데이터 변환
                const grouped: Record<Importance, TransformBarData> = {} as Record<Importance, TransformBarData>;
                ...
                const sorted = Object.values(grouped).sort((a, b) => a.importance - b.importance);
                setChartData(sorted);
            }
            catch {
                ...
                <>
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={chartData} barCategoryGap="20%">
                            <XAxis dataKey="importance" label={{ value: "중요도", position: "insideBottom", offset: -5 }} />
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={LinechartData} margin={{ top: 20, right: 40, left: 20, bottom: 20 }}>
                            ...
                            <XAxis
                                ...
                                label={{ value: "중요도", position: "insideBottom", offset: -5 }}
                            />
                            <YAxis
